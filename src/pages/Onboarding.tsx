...
import AgreementModal, { TERMS_VERSION } from "@/components/AgreementModal";

type Step = "role" | "user_terms" | "hauler_terms" | "profile" | "vehicle";
...
const Onboarding = () => {
  ...
  const handleRoleSelect = (selected: "user" | "hauler") => {
    setRole(selected);
    setStep("user_terms"); // Always show general terms first
  };

  // ── Step 2a: General terms accepted ────────────────────────────────────────
  const handleUserTermsAccepted = async () => {
    if (!user) return;
    try {
      await supabase
        .from("profiles")
      ...
      // Don't block onboarding if DB write fails — acceptance is re-checked on next login
    }
    setStep("profile");
  };

  // ── Step 2b: Hauler contractor terms accepted ───────────────────────────────
  const handleHaulerTermsAccepted = async () => {
    if (!user) return;
    try {
      await supabase
        .from("profiles")
        .update({ hauler_terms_accepted_at: new Date().toISOString() })
        .eq("user_id", user.id);
    } catch (err) {
      console.error("Failed to save hauler terms acceptance:", err);
    }
    setStep("vehicle");
  };
  ...
  const handleProfileSubmit = async () => {
    ...
    try {
      ...
      if (role === "hauler") {
        setStep("hauler_terms"); // Show hauler-specific contractor agreement
      } else {
        toast.success("Welcome to YeeHaul!");
  ...
  // ── Step 4 (hauler only): Vehicle info ─────────────────────────────────────
  const handleVehicleSubmit = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success("You're all set! Welcome to YeeHaul!");
      window.location.href = "/dashboard";
    } catch (err: any) {
      toast.error(err.message);
    } finally {
  ...
  return (
    <>
      ...
      <AgreementModal
        type="user"
        open={step === "user_terms"}
        onAccepted={handleUserTermsAccepted}
      />

      {/* Hauler-specific contractor agreement modal */}
      <AgreementModal
        type="hauler"
        open={step === "hauler_terms"}
        onAccepted={handleHaulerTermsAccepted}
      />
      ...
      <div className="min-h-screen flex items-center justify-center px-4 bg-background">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              {step === "role" && "How will you use YeeHaul?"}
            ...
            {step === "vehicle" && (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    placeholder="e.g., 2020 Ford F-150, 6ft bed"
                    value={vehicleDesc}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    placeholder="e.g., Up to 2000 lbs, 8ft bed"
                    value={capacity}
